...
public class RunnerGamePowerupSpawner : MonoBehaviour {
	...
	{
		float[] heights = { 2.5f, 3.9f };
		int idx = Random.Range(0, 2);
		// spawn correct
		SpawnPowerup(combo.shape, new Vector3(world.activeArea, heights[idx], 0f));
		...
		SpawnPowerup((Combo.Shape)c, new Vector3(world.activeArea, heights[(idx+1)%heights.Length], 0f));
	}
